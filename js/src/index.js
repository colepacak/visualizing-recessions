...
lights.name = 'lights';
var light1 = new THREE.PointLight(0xffffff, 1, 400, 1);
var light2 = new THREE.PointLight(0xffffff, 1, 400, 2);
var light3 = new THREE.PointLight(0xffffff, 1, 400, 1);
...
function fetchData() {
  return fetch(config.recessionDataUri[process.env.NODE_ENV])
    .then(function(resp) {
      return resp.json();
    })
    .then(function(resp) {
...
function addGridLine(start, end, axis) {
  return new Promise(function(resolve, reject) {
    var material = new THREE.LineBasicMaterial({ color: colorLabel });
